 ...
 */
const fitStringToSize = (str: string, size: number): string => {
    let result = str;
 ...
 */
const calcUniqueString = (str: string): string => {
    const seed = str
        .toLowerCase()
        .split("")
 ...
 * @return The unique value.
 */
const calcUserUniqueString = (user: User): string =>
    calcUniqueString(toFullName(user));
